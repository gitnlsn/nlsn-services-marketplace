...
export const releaseEscrowFundsInputSchema = z.object({
	paymentId: z.string().cuid(),
});
...
export type ReleaseEscrowFundsInput = z.infer<typeof releaseEscrowFundsInputSchema>;

interface PaymentRouterServiceDependencies {
	db: PrismaClient;
	currentUser?: User;
}

export class PaymentRouterService {
	constructor(private deps: PaymentRouterServiceDependencies) {}
	...
	async processPayment(input: ProcessPaymentInput) {
		...
		if (booking.clientId !== this.deps.currentUser.id) {
			throw new TRPCError({
				code: "FORBIDDEN",
		...
		try {
			...
			const updatedPayment = await this.deps.db.payment.update({
	...
	async getPaymentStatus(input: GetPaymentStatusInput) {
		...
		const payment = await this.deps.db.payment.findUnique({
		...
		if (
			payment.booking.clientId !== this.deps.currentUser.id &&
			payment.booking.providerId !== this.deps.currentUser.id
		) {
			throw new TRPCError({
				code: "FORBIDDEN",
	...
	async requestRefund(input: RequestRefundInput) {
		...
		if (payment.booking.clientId !== this.deps.currentUser.id) {
			throw new TRPCError({
				code: "FORBIDDEN",
		...
		try {
			...
			await this.deps.db.$transaction([
				this.deps.db.notification.create({
					data: {
...
export function createPaymentRouterService(deps: PaymentRouterServiceDependencies): PaymentRouterService {
	return new PaymentRouterService(deps);
}